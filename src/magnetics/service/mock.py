...
_PROFILE = {
    "MOCK-A": dict(
        label="MOCK-A",
        note="fake · dense array · m/n=2/1 LOCKED · well-conditioned (K~7)",
        K=6.73,
        chi2=0.55,
        m_max=4,
        mode=(2, 1),
    ...
    "MOCK-B": dict(
        label="MOCK-B",
        note="fake · sparse legacy array · ROTATING n=1 · ill-conditioned (K~21)",
        K=21.2,
        chi2=0.30,
        m_max=1,
        mode=(1, 1),
...
MACHINES = [
    {"id": k, "label": v["label"], "device": "synthetic", "note": v["note"], "mock": True}
...
def _geometry(machine: str) -> dict:
    ...
    return {
        "sensors": [diiid.sensor(n, _GEOM_REF_SHOT) for n in spec["sensors"]],
        "arrays": spec["arrays"],
    }
    ...
    for i, (nphi, nth) in enumerate(resolutions):
        ...
        frames.append(
            (
                ...
                {
                    "contour": {
                        "phi": np.round(phi, 1).tolist(),
                        "theta": np.round(theta, 1).tolist(),
    ...
    for i, (nt, nf) in enumerate(resolutions):
        ...
        frames.append(
            (
                ...
                {
                    "spectrogram": {
                        "t_ms": np.round(t, 1).tolist(),
                        "f_kHz": np.round(f, 2).tolist(),
                    ...
                    "coherence": {
                        "f_kHz": np.round(f, 2).tolist(),
                        "coh": np.round(
                            np.clip(0.9 - 0.012 * f + rng.normal(0, 0.03, f.size), 0, 1), 3
                        ).tolist(),
                    },
                },
            )
