...
| local module | role | OMFIT source (`../OMFIT-magnetics/`) |
|---|---|---|
| `io_data.py` | `load_shot` reads RAW / PLASMA_PARAMS / COUPLING netCDF (the "fetch" step) | `SCRIPTS/fetch_magnetics.py` |
| `prep.py` | trim, DC-comp, integrate, causal filter, detrend, SVD-condition | `SCRIPTS/prep_magnetics.py` |
| `fit.py` | basis matrix + SVD least-squares modal fit | `SCRIPTS/fit_magnetics.py` |
| `run.py` | `run_steps` orchestrator (load → prep → fit) | `SCRIPTS/run_magnetics.py` |
| `plots.py` | sensor map, signals, fit quality, mode amp/phase, φ-vs-time contour, SVD diagnostics | `PLOTS/plot_magnetics_*.py` |
| `omfit_compat.py` | shim for OMFIT-runtime symbols (`printi`, `cornernote`, `uband`, `is_device`, channel filters, wall) | (OMFIT framework) |

Sensor geometry, channel-name filters and the first-wall outline are read directly from the copied
OMFIT module's `DATA/DIII-D/` tables (no duplication).
...
`channel_filter` accepts a regex, a list of regexes, or a friendly name from
`DATA/DIII-D/channel_filters.txt` (e.g. `'Bp_LFS_midplane'`, `'Bp_LFS_R+1'`, `'Bp_All'`).
